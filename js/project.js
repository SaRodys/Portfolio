...
const titleToClassMap = {
  ...
  "REGIA E MONTAGGIO": "regia",
  "PROGETTAZIONE APP": "app",
  "SOCIAL MEDIA MANAGEMENT": "social",
  "SCENOGRAFIA TEATRALE": "scenografia",
  "GRAFICA 3D": "grafica"
};
...
cards.forEach(card => {
  card.addEventListener("click", () => {
    const title = card.querySelector(".slider-title p").textContent.trim();
    const targetClass = titleToClassMap[title];

    if (!targetClass) return;

    const sections = document.querySelectorAll("section.items");

    if (activeClass === targetClass) {
